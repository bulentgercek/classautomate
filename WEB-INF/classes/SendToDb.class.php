...
class SendToDb
{
	...
	public static function add($object, $nonPOST=array())
	{
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->add() - addCriteria : ";
	...
	public static function update($object, $nonPOST=array())
	{
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->update() - commonDbValues : ";
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->update() - arrayDifference : ";
		...
		if ($arrayDifference != NULL) {
			...
			foreach ($arrayDifference as $key => $value) {
				$i++;
				if ($i != count($arrayDifference)) { $comma = ',';
				} else { $comma = '';
				}
				$set .= $key . '=\'' . $value . '\'' . $comma;
				$columnsBackup[] = $key;
			 ...
			 * veritabanina ekle
			 */
			if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->update() - updateCriteria : ";
	...
	public static function delete($object, $nonPOST=array())
	{
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->delete() - commonDbValues : ";
			var_dump($commonDbValues);
		}
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->delete() - deleteCriteria : ";
	...
	public static function arrayImplement($object, $nonPOST=array(), $process)
	{
		...
		if (empty($nonPOST)) $infoArray = $_POST; else $infoArray = $nonPOST;

		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->arrayImplement() - infoArray : ";
			var_dump($infoArray);
		}
		/**
		...
		if (debugger("SendToDb")) { echo "DEBUG : " . getCallingClass() . "->SendToDb->arrayImplement() - valuesArray : ";
