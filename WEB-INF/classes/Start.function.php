...
function getWeekNumberOfDate($date = 'now') 
{
	if ($date == 'now') {
		...
		$day = $expFormattedDate[2]; $month = $expFormattedDate[1]; $year = $expFormattedDate[0];
	}
...
function getDateTimeDiff($firstValue, $secondValue, $format, $debug = false)
{
    ...
    if ($format == 'y') return $Difference->y;
    if ($format == 'm') return $Difference->m;
	if ($format == 'w') {
		$weekDiffTemp = strtotime($firstValue) - strtotime($secondValue);
		$weekDiff = floor($weekDiffTemp / 604800);
		return $weekDiff;
	}
    if ($format == 'd') return $Difference->d;
    if ($format == 'h') return $Difference->h;
    if ($format == 'i') return $Difference->i;
    if ($format == 's') return $Difference->s;
	if ($format == 'type') {
