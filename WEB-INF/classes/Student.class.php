...
class Student extends Person
{
	...
	public function getPaymentAndPeriodChanges()
	{
		...
		if (count($paymentChanges) > count($paymentPeriodChanges)) {
            ...
            if ($whoIsVeryLast == 0) $result = $paymentChanges;
			if ($whoIsVeryLast == 1) $result = $paymentChanges;
			if ($whoIsVeryLast == -1) $result = $paymentPeriodChanges;
		}
	...
	public function getPaymentAndPeriodChangesByClassroom($Classroom)
	{
		...
		if (debugger('Student')) echo '<br>->Tum veriler : ';
		if (debugger('Student')) var_dump($paymentAndPeriodChanges);
		if (debugger('Student')) echo '<br>->Istenilen veriler : ';
		$classroomCode = $Classroom->getInfo('code');
		$currentKey = 0;
		...
		foreach ($paymentAndPeriodChanges as $key => $value) {
            ...
            if (debugger('Student')) var_dump($key);
			/**
			...
			if ($codeKey != '') {
				...
				if (debugger('Student')) var_dump('Previous : ' . $previousPaymentValue . ' - ' . $previousPaymentPeriodValue);
				if (debugger('Student')) var_dump('Current : ' . $currentPaymentValue . ' - ' . $currentPaymentPeriodValue);
				...
				if ($currentPaymentValue != $previousPaymentValue || $currentPaymentPeriodValue != $previousPaymentPeriodValue) {
					...
					if ($previousState == 'start') {
						 ...
						 * yeni kaydi baslat
						 */
						if (debugger('Student')) echo "Bu arada onceki kayit kapandi. Yeni kayit var aciyorum.<br>";
						$newStart = true;
					 ...
					 */
					    ...
					    if (debugger('Student')) echo "Kayit baslatilmamis. Temiz bir tane aciyorum.<br>";
						$addState = 'start';
					}
								
				} else {
				    if (debugger('Student')) echo "Önceki kayıtlarla sonuc tamamen ayni. Hic birsey yapma.<br>";
				}
			 ...
			 */
                ...
                if (debugger('Student')) echo "Kayıt istenilen sınıf değil!<br>";
				$isAddOk = false;
			}
			...
			if ($isAddOk) {
				...
				if ($addState == 'start') {
					if (count($finalList) > 0) $currentKey++;
            ...
            if ($key == count($paymentAndPeriodChanges)-1) {
                if (debugger('Student')) echo "Son Kayıt geldi.<br>";
                if ($previousState == 'start') {
                    if (debugger('Student')) echo "Acik kayit varmis kapatiyorum.<br>";
				...
				if ($senderMethod == "add" || $senderMethod == "update") {
					...
					if ($this->isClassroomSbyRoomInPost()) {
						...
						foreach ($classroomStatusList as $key => $value) {
							...
							if ($key < count($classroomStatusList)-1) $statusResult .= ',';
						}
						...
						foreach ($newClassrooms as $key => $value) {
							...
							if ($Classroom->getInfo('status') == 'notUsed')
								$Classroom->setInfo( array('status'=>'used', 'tc:update'=>'updateClassroomForm|direct') );
							if ($key < count($newClassrooms)-1) $statusResult .= ',';
						}
