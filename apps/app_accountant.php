 ...
 */
if ($_GET['mainSelect'] == 'income')
		$typeCode = '+';
if ($_GET['mainSelect'] == 'expense')
		$typeCode = '-';
if ($_GET['mainSelect'] == 'profit')
		$typeCode = '=';
if (!isset($_GET['mainSelect']))
		$typeCode = '+';
/**
 ...
 */
if (isset($_GET['subTypeSelection']))
		$intend['where'] .= " AND subType='" . $_GET['subTypeSelection'] . "'";

if (isset($_GET['startDate'])) {
		$intend['where'] .= " AND `dateTime` BETWEEN '" . $date($_GET['startDate']) . " 00:00:00'";
} else {
		if ($typeCode != '=') {
				$startDateWithDay1 = getDateTime('%Y-%m-') . '01';
				$intend['where'] .= " AND `dateTime` BETWEEN '" . $date($startDateWithDay1) . " 00:00:00'";
		}
}

if (isset($_GET['endDate'])) {
		$intend['where'] .= " AND '" . $date($_GET['endDate']) . " 23:59:59'";
} else {
		if ($typeCode != '=') {
				$intend['where'] .= " AND '" . $date('now') . " 23:59:59'";
		}
}
/**
