	...
	$(function() {
		...
		$.addUpdateRow = function() {
			...
			$("#paymentPeriod option").each(function(index) {
				if (tableRowPaymentPeriod == $(this).val())	isSelected = "SELECTED"; else isSelected = "";
			...
			if ($('#classListTable tr').length == 3) { $('#listingWithHeader').slideToggle(); }
		...
		$.removeUpdateRow = function(id) {
			...
			if ($('#classList option').length > 0) { $('#selectAdd').slideDown(); }
			
			// varsa sinif listesindeki splitter'i disable et
			$.disableSplitter('classList');
	
			// sinif sayisi bilgilendirme alanina icerik girisi yap
			if ($('#classCounter').val() < 1) $('#classCounterInfo, #vres_classCounter').show();
				else $('#classCounterInfo, #vres_classCounter').hide();
