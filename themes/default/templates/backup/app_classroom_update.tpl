...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#updateClassroomForm #instructor').val('{$classroomValues["instructor"]}');
		$('#updateClassroomForm #instructorPaymentPeriod').val('{$classroomValues["instructorPaymentPeriod"]}');
	     ...
	     */
	    $('#updateClassroomForm').append('<input type="hidden" name="formName" value="updateClassroomForm" />');
	     ...
	     */
	    $('#updateClassroomForm').append('<input type="hidden" name="subCodeSelect" id="subCodeSelect" />');
	    
 	    /**
	     * sinif dayTime gizli day/time/endtime nesneleri
	     */
	    $('#updateClassroomForm').append('<input type="hidden" name="subCodeArray" id="subCodeArray" />');
	    $('#updateClassroomForm').append('<input type="hidden" name="dayArray" id="dayArray" />');
	    $('#updateClassroomForm').append('<input type="hidden" name="timeArray" id="timeArray" />'); 
	    $('#updateClassroomForm').append('<input type="hidden" name="endTimeArray" id="endTimeArray" />');
        ...
        $('#updateClassroomForm').live('submit', function() {
        	/** tablo bilgilerini gizli form nesnelerine aktar */
        	$.tableToForm();
		...
		$('[id^="removeBut"]').live("click", function() {
			...
			$.dayTimeTableControl.remove(this);
		});
		...
		$.addUpdateRow = function() {
			...
			if ($.dayTimeControl()) { 
				...
				var tableRowButtonValue = $('#daySelect :selected').val() + "_" + tableRowTime.replace(/:/g, "") + "_" + tableRowEndTime.replace(/:/g, "") + "_" + classroom;
		...
		/**
		 * table uzerindeki bilgileri form nesnelerine aktar 
		 */
		$.tableToForm = function() {
			...
			$('#dayTimeListTable tr').each(function() {
				count++;
				if (count > 2) {
    				dayArray.push($(this).find('a').attr('id').split('_')[1]);
    				timeArray.push($(this).find("td").eq(1).html());
    				endTimeArray.push($(this).find("td").eq(2).html());
    			}
			});
			
			/** gizli form degiskenlerine bilgileri ata */
			$('#dayArray').val(dayArray);
			$('#timeArray').val(timeArray);
			$('#endTimeArray').val(endTimeArray);
		}
		
		/**
		 * database verilerine ":" ekle 
		 */
		$.timesFromDb = function(string) {
		 ...
		 */	
		$.subCodeUpdateIndex = function(theValue) {
			$('#subCodeSelect').val(theValue).trigger('change');
		}
		...
		for (i=0; i<splittedDays.length; i++) {
			...
			$.timeUpdateIndex( splittedTimes[i] );
			$.endTimeUpdateIndex( splittedEndTimes[i] );
			$.addUpdateRow();	
		}
        ...
        } else if (termDateLimit != "0000-00-00") {
        	$.termStatusSelectUpdateIndex("checked");
	...
	/**
	 * Bu kısım TimePicker'ın yazarı François Gélinas'ın sitesindeki örnekten alınmıştır. (By François Gélinas)
	 */
            ...
            <div id="formInner">
	            ...
	            <div id="formLine">
	            	<div id="payment">
	                	...
	                	<input id="instructorPayment" name="instructorPayment" type="text" size="1" maxlength="3" tabindex="{$tabStart++}" class="gTextInput" value="0">
	                </div>
				...
				<div style="display:none" id="dialogUpdate">{$app_classroom_update_sureToUpdate}</div>
